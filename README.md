...
```bash
cd hro-operations
poetry install                      # psycopg + 依存(backtest/buyer とその推移的依存)
```

モノレポの兄弟(`../hro-backtest`,`../hro-buyer`)が同じ階層に存在する前提(path 依存)。
Windows でも同様に配置して `poetry install`。`hro-ops agent`(sync/odds)は betting スタックを
import しないが、依存としてはインストールされる。
